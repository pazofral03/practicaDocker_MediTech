    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if fecha_hora < datetime.now():
                flash('La fecha y hora no pueden ser anteriores a la actual.', 'danger')
        ...
        cita_existente = Cita.query.filter_by(medico_id=medico_id, fecha_hora=fecha_hora).first()
        if cita_existente:
             if cita_existente.estado != 'Cancelada':
                flash('El médico ya tiene una cita agendada en ese horario. Por favor selecciona otra hora.', 'danger')
                return redirect(url_for('citas.solicitar'))
...
def gestionar(id):
        ...
        if current_user.rol in ['medico', 'admin']:
            ...
            if nueva_fecha and nueva_hora:
                try:
                    ...
                    if nueva_fecha_hora != cita.fecha_hora: # Solo si cambia
                        # Validaciones
                        if nueva_fecha_hora < datetime.now():
                             flash('La nueva fecha y hora no pueden ser anteriores a la actual.', 'danger')
                             return redirect(url_for('citas.gestionar', id=id))
                             
                        # Solapamiento
                        overlap = Cita.query.filter(
                        ...
                        if overlap:
                            flash('El médico ya tiene una cita en ese nuevo horario.', 'danger')
                            return redirect(url_for('citas.gestionar', id=id))
                            
                        cita.fecha_hora = nueva_fecha_hora
                except ValueError:
                    flash('Formato de fecha u hora inválido al modificar.', 'danger')
                    return redirect(url_for('citas.gestionar', id=id))
    ...
    return render_template('cita_gestion.html', cita=cita, now_date=datetime.now().strftime('%Y-%m-%d'))
